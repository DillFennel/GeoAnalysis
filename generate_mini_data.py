            ...
            for i in data_from_mos['features']:
                ...
                for cell_id in cell_id_list:
                    if not(cell_id in data.keys()):
                        ...
                        data[cell_id][data_set['name']] += 1
        print("В дата сете "+ data_set['name'] + " было "+str(errors)+" немосковских ячеек и "+str(not_state)+" пустых координат")
print("data заполнена, записываем в файл")
with open('mini_data.json', 'w') as f:
    json.dump(data, f)
print("Готово")
